#!/usr/bin/perl
#
# This was before I knew about use strict so most variables are global.
#
...
sub update {
    ...
    print "Press H for controls (capital).";
    print "\e[?25h";
}
...
sub search {
    ...
    # Double check if user is trying to search a square flagged as a mine.
    #
    # We handle getting the next input here just for the sake of clearing the
    # message...
    if ( $main[$cols*$_[1]+$_[0]] == 10 && $first_search == 1) {
        $first_search=0;
        print "\e[".($rows+4).";1HAre you sure? (s to search)\e[K\n";
        print "\e[${y};".(${x}*2-1)."H";

        $input = getc(TTY);
           if ($input eq "A" || $input eq "k") { if ( $y > 1 ) { $y--; } }
        elsif ($input eq "B" || $input eq "j") { if ( $y < $rows ) { $y++; } }
        elsif ($input eq "C" || $input eq "l") { if ( $x < $cols ) { $x++; } }
        elsif ($input eq "D" || $input eq "h") { if ( $x > 1 ) { $x--; } }
        elsif ($input eq "q") { end_game; }
        elsif ($input eq "f") { flag ($x-1, $y-1); update; check_for_win; }
        elsif ($input eq "s") { search ($x-1, $y-1); }
...
sub controls {
    $PAUSE_TIME=time;
    print "\e[2J\e[H";
    print "Movement : arrow keys or vim-style.\n";
    print "Flag     : f\n";
    print "Search   : s\n";
